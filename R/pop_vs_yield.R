...
pop_v_yield <- function(z.vec = c(1, 2.39, 5.99),
  ...
  #unpr <- NPROut$npr
  Fec0 <- 1.0/N0
  fmax <- getFecmax2(lambdaMax = lambdaMax,
                     S1plus = S1plus,
                     S0 = S0,
                     AgeMat = AgeMat)
  A <- (fmax - Fec0) / Fec0
  ...
  if(ggp){
    ...
    for(j in 1:length(z.vec)){
      ...
      u1p <- NPR(S0 = S0,S1plus = S1plus,nages = nages,AgeMat = AgeMat,f = 0)$P1r *
        getRF(E = 0,S0 = S0,S1plus = S1plus,nages = nages,K1plus = K1plus,AgeMat = AgeMat,z = z,A = A,P0 = P0,N0 = N0) # == 1
      ...
      for(i in 1:length(E.vec)){
        ...
        n1p <- NPR(S0 = S0,S1plus = S1plus,nages = nages,AgeMat = AgeMat,f = E.vec[i])$P1r *
          getRF(E = E.vec[i],S0 = S0,S1plus = S1plus,nages = nages,K1plus = K1plus,AgeMat = AgeMat,z = z,A = A,P0 = P0,N0 = N0)
